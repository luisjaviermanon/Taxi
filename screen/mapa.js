...
import PlaceInput from '../components/DestinationButton'
import {
  BASE_URL,
  API_KEY,
  getRoute,
...
import TAXI_LOGO from "../assets/car.png";
...
const PassengerScreen = props => {
  ...
  const { container, mapStyle, taxiStyle,p } = styles;
  useEffect(() => {
    if (taxiCoords) {
    ...
    <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
      <View style={container}>
        ...
        >
          ...
          {destinationCoords && <Marker coordinate={destinationCoords} image={markerImage}  anchor={{ x: 0, y: 0 }}/>}
          {taxiCoords && (
